...
def generate_sample_data(n_samples: int = 1000, n_features: int = 4, random_state: int = 42) -> pd.DataFrame:
    """
...
def preprocess_data(df: pd.DataFrame, target_col: str = 'target') -> Tuple[np.ndarray, np.ndarray, StandardScaler]:
    """
...
def save_processed_data(X_train: np.ndarray, X_test: np.ndarray,
                       ...
                       scaler: StandardScaler, output_path: str = 'data/processed/') -> None:
    """
